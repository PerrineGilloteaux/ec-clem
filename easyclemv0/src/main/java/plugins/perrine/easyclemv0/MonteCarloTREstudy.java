...
public class MonteCarloTREstudy extends EzPlug implements EzStoppable {
	...
	private TransformationComputer transformationComputer = new TransformationComputer();
	...
	protected void execute() {
		...
		for(int i = 0; i < sourceDataset.getN(); i++) {
			...
			for (int mc = 0; mc < nbsimul; mc++) {
				...
				Similarity similarity = transformationComputer.compute(sourceDatasetWithNoise, targetDatasetWithNoise);
