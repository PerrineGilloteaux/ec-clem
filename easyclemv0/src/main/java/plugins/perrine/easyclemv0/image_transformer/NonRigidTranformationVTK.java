...
import plugins.perrine.easyclemv0.storage.xml.non_rigid.NonRigidTransformationXmlWriter;
...
public class NonRigidTranformationVTK implements ImageTransformerInterface {
    ...
    private NonRigidTransformationXmlWriter nonRigidTransformationXmlFileWriter;
...
//        );
        ...
        vtkPolyData modifiedpoints = apply(lmsource, myvtkTransform);

        for (int p = 0; p < modifiedpoints.GetNumberOfPoints(); p++) {
            double[] newpos = modifiedpoints.GetPoint(p);

            fiducialSet.getSourceDataset().getMatrix().set(p, 0, newpos[0] / this.spacingx);
            fiducialSet.getSourceDataset().getMatrix().set(p, 1, newpos[1] / this.spacingx);
            fiducialSet.getSourceDataset().getMatrix().set(p, 2, newpos[2] / this.spacingx);
        }
